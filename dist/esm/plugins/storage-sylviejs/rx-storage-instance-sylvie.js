...
import { closeSylvieCollections, getSylvieDatabase, OPEN_SYLVIEJS_STORAGE_INSTANCES, SYLVIEJS_COLLECTION_DEFAULT_OPTIONS, stripSylvieKey, getSylvieSortComparator, getSylvieLeaderElector, requestRemoteInstance, mustUseLocalState, handleRemoteRequest, RX_STORAGE_NAME_SYLVIEJS, transformRegexToRegExp } from "./sylviejs-helper.js";
...
var instanceId = now();
export var RxStorageInstanceSylvie = /*#__PURE__*/function () {
  ...
  _proto.bulkWrite = async function bulkWrite(documentWrites, context) {
    ...
    documentWrites.forEach(writeRow => {
      ...
      if (documentInDb) {
        ...
        docsInDb.set(id, stripSylvieKey(documentInDb));
      }
    });
    ...
    categorized.bulkUpdateDocs.forEach(writeRow => {
      ...
      var documentInDbWithSylvieKey = getFromMapOrThrow(docsInDbWithSylvieKey, docId);
  ...
  _proto.query = async function query(preparedQueryOriginal) {
    ...
    var foundDocuments = sylvieQuery.data().map(sylvieDoc => stripSylvieKey(sylvieDoc));
  ...
  _proto.close = async function close() {
    ...
    this.closed = (async () => {
      ...
      if (this.internals.localState) {
        ...
        var dbState = await getSylvieDatabase(this.databaseName, this.databaseSettings);
        await dbState.saveQueue.run();
        await closeSylvieCollections(this.databaseName, [localState.collection]);
      }
    })();
    return this.closed;
  };
...
export async function createSylvieLocalState(params, databaseSettings) {
  if (!params.options) {
    params.options = {};
  }
  var databaseState = await getSylvieDatabase(params.databaseName, databaseSettings);
...
export async function createSylvieStorageInstance(storage, params, databaseSettings) {
  var internals = {};
  var broadcastChannelRefObject = {};
  if (params.multiInstance) {
    var leaderElector = getSylvieLeaderElector(params.databaseInstanceToken, broadcastChannelRefObject, params.databaseName);
  ...
  await addRxStorageMultiInstanceSupport(RX_STORAGE_NAME_SYLVIEJS, params, instance, internals.leaderElector ? internals.leaderElector.broadcastChannel : undefined);
  if (params.multiInstance) {
    /**
