import { createSylvieLocalState } from "./rx-storage-instance-sylvie.js";
...
import { SylvieSaveQueue } from "./sylvie-save-queue.js";
...
export var SYLVIE_BROADCAST_CHANNEL_MESSAGE_TYPE = 'rxdb-sylviejs-remote-request';
export var SYLVIE_KEY_OBJECT_BROADCAST_CHANNEL_MESSAGE_TYPE = 'rxdb-sylviejs-remote-request-key-object';
export var RX_STORAGE_NAME_SYLVIEJS = 'sylviejs';

/**
 * Sylvie attaches a $loki property to all data
 * which must be removed before returning the data back to RxDB.
 */
export function stripSylvieKey(docData) {
  if (!docData.$loki) {
    return docData;
  }
 ...
 */
export var OPEN_SYLVIEJS_STORAGE_INSTANCES = new Set();
export var SYLVIEJS_COLLECTION_DEFAULT_OPTIONS = {
...
export function getSylvieDatabase(databaseName, databaseSettings) {
  return getFromMapOrCreate(SYLVIE_DATABASE_STATE_BY_NAME, databaseName, () => {
    /**
    ...
    var databaseState = (async () => {
      ...
      var database = new Sylvie(databaseName + '.db', flatClone(useSettings));
      var sylvieSaveQueue = new SylvieSaveQueue(database, useSettings);
      ...
      var unloads = [];
      if (hasPersistence) {
        unloads.push(unloadAdd(() => sylvieSaveQueue.run()));
      }
      var state = {
        database,
...
export async function closeSylvieCollections(databaseName, collections) {
 ...
 */
export function getSylvieSortComparator(_schema, query) {
  if (!query.sort) {
...
export function getSylvieLeaderElector(databaseInstanceToken, broadcastChannelRefObject, databaseName) {
  var broadcastChannel = getBroadcastChannelReference(RX_STORAGE_NAME_SYLVIEJS, databaseInstanceToken, databaseName, broadcastChannelRefObject);
...
export async function mustUseLocalState(instance) {
  ...
  if (leaderElector.isLeader && !instance.internals.localState) {
    ...
    instance.internals.localState = createSylvieLocalState({
