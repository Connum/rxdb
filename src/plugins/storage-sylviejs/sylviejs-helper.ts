...
import type {
    ...
    SylvieDatabaseState,
    SylvieLocalDatabaseState,
    SylvieRemoteResponseBroadcastMessage,
...
import { SylvieSaveQueue } from './sylvie-save-queue.ts';
...
export const SYLVIE_BROADCAST_CHANNEL_MESSAGE_TYPE = 'rxdb-sylviejs-remote-request';
export const SYLVIE_KEY_OBJECT_BROADCAST_CHANNEL_MESSAGE_TYPE = 'rxdb-sylviejs-remote-request-key-object';
export const RX_STORAGE_NAME_SYLVIEJS = 'sylviejs';

/**
 * Sylvie attaches a $loki property to all data
 * which must be removed before returning the data back to RxDB.
 */
export function stripSylvieKey<T>(docData: RxDocumentData<T> & { $loki?: number; }): T {
    if (!docData.$loki) {
        return docData;
    }
 ...
 */
export const OPEN_SYLVIEJS_STORAGE_INSTANCES: Set<RxStorageInstanceSylvie<any>> = new Set();


export const SYLVIEJS_COLLECTION_DEFAULT_OPTIONS: Partial<any> = {
...
const SYLVIE_DATABASE_STATE_BY_NAME: Map<string, Promise<SylvieDatabaseState>> = new Map();
export function getSylvieDatabase(
    databaseName: string,
    databaseSettings: SylvieDatabaseSettings
): Promise<SylvieDatabaseState> {


    return getFromMapOrCreate(
        SYLVIE_DATABASE_STATE_BY_NAME,
        databaseName,
        () => {
            /**
            ...
            const databaseState = (async () => {
                ...
                const database = new Sylvie(
                    databaseName + '.db',
                    flatClone(useSettings)
                );
                const sylvieSaveQueue = new SylvieSaveQueue(
                    database,
                    useSettings
                );
                ...
                if (hasPersistence) {
                    unloads.push(
                        unloadAdd(() => sylvieSaveQueue.run())
                    );
                }

                const state: SylvieDatabaseState = {
                    database,
...
export async function closeSylvieCollections(
    databaseName: string,
    collections: any[]
) {
    const databaseState = await SYLVIE_DATABASE_STATE_BY_NAME.get(databaseName);
    if (!databaseState) {
        // already closed
        return;
    }
 ...
 */
export function getSylvieSortComparator<RxDocType>(
...
export async function requestRemoteInstance(
    instance: RxStorageInstanceSylvie<any>,
    operation: string,
    params: any[]
): Promise<any | any[]> {
    const isRxStorageInstanceSylvie = typeof (instance as any).query === 'function';
    const messageType = isRxStorageInstanceSylvie ? SYLVIE_BROADCAST_CHANNEL_MESSAGE_TYPE : SYLVIE_KEY_OBJECT_BROADCAST_CHANNEL_MESSAGE_TYPE;
...
export async function handleRemoteRequest(
    ...
    if (
        msg.type === SYLVIE_BROADCAST_CHANNEL_MESSAGE_TYPE &&
        msg.requestId &&
        ...
        const response: SylvieRemoteResponseBroadcastMessage = {
            response: true,
...
export async function mustUseLocalState(
    instance: RxStorageInstanceSylvie<any>
): Promise<SylvieLocalDatabaseState | false> {
    if (instance.closed) {
        /**
    ...
    if (
        ...
        instance.internals.localState = createSylvieLocalState<any>({
            databaseInstanceToken: instance.databaseInstanceToken,
            databaseName: instance.databaseName,
            collectionName: instance.collectionName,
            options: instance.options,
            schema: (instance as RxStorageInstanceSylvie<any>).schema,
