...
import {
    closeSylvieCollections,
    getSylvieDatabase,
    OPEN_SYLVIEJS_STORAGE_INSTANCES,
    SYLVIEJS_COLLECTION_DEFAULT_OPTIONS,
    stripSylvieKey,
    getSylvieSortComparator,
    getSylvieLeaderElector,
    requestRemoteInstance,
    mustUseLocalState,
    handleRemoteRequest,
    RX_STORAGE_NAME_SYLVIEJS,
    transformRegexToRegExp
} from './sylviejs-helper.ts';
...
export class RxStorageInstanceSylvie<RxDocType> implements RxStorageInstance<
    RxDocType,
    SylvieStorageInternals,
    SylvieSettings,
    RxStorageDefaultCheckpoint
> {
    ...
    constructor(
        ...
        public readonly storage: RxStorageSylvie,
        public readonly databaseName: string,
        public readonly collectionName: string,
        public readonly schema: Readonly<RxJsonSchema<RxDocumentData<RxDocType>>>,
        public readonly internals: SylvieStorageInternals,
        public readonly options: Readonly<SylvieSettings>,
        public readonly databaseSettings: SylvieDatabaseSettings
    ) {
        this.primaryPath = getPrimaryFieldOfPrimaryKey(this.schema.primaryKey);
        OPEN_SYLVIEJS_STORAGE_INSTANCES.add(this);
    ...
    async bulkWrite(
        ...
        const docsInDbWithSylvieKey: Map<
        ...
        documentWrites.forEach(writeRow => {
            ...
            if (documentInDb) {
                docsInDbWithSylvieKey.set(id as any, documentInDb);
                docsInDb.set(id as any, stripSylvieKey(documentInDb));
            }
        });
        ...
        categorized.bulkUpdateDocs.forEach(writeRow => {
            ...
            const documentInDbWithSylvieKey = getFromMapOrThrow(docsInDbWithSylvieKey, docId as any);
    ...
    async findDocumentsById(ids: string[], deleted: boolean): Promise<RxDocumentData<RxDocType>[]> {
        ...
        ids.forEach(id => {
            ...
            if (
                ...
                ret.push(stripSylvieKey(documentInDb));
            }
        });
        return ret;
    }
    async query(preparedQueryOriginal: PreparedQuery<RxDocType>): Promise<RxStorageQueryResult<RxDocType>> {
        ...
        let sylvieQuery = localState.collection
            .chain()
            .find(mustRunMatcher ? {} : preparedQuery.selector);

        if (preparedQuery.sort) {
            sylvieQuery = sylvieQuery.sort(getSylvieSortComparator(this.schema, preparedQuery));
        }
        ...
        let foundDocuments = sylvieQuery.data().map((sylvieDoc: any) => stripSylvieKey(sylvieDoc));
    ...
    async close(): Promise<void> {
        ...
        this.closed = (async () => {
            ...
            OPEN_SYLVIEJS_STORAGE_INSTANCES.delete(this);
            if (this.internals.localState) {
                const localState = await this.internals.localState;
                const dbState = await getSylvieDatabase(
                    this.databaseName,
                    this.databaseSettings
                );
                await dbState.saveQueue.run();
                await closeSylvieCollections(
                    this.databaseName,
                    [
...
export async function createSylvieLocalState<RxDocType>(
    params: RxStorageInstanceCreationParams<RxDocType, SylvieSettings>,
    databaseSettings: SylvieDatabaseSettings
): Promise<SylvieLocalDatabaseState> {
    if (!params.options) {
        params.options = {};
    }

    const databaseState = await getSylvieDatabase(
        params.databaseName,
        databaseSettings
    );
    ...
    const sylvieCollectionName = params.collectionName + '-' + params.schema.version;
    const collectionOptions: Partial<any> = Object.assign(
        {},
        sylvieCollectionName,
        {
    ...
    const ret: SylvieLocalDatabaseState = {
        databaseState,
        collection
    };
...
export async function createSylvieStorageInstance<RxDocType>(
    storage: RxStorageSylvie,
    params: RxStorageInstanceCreationParams<RxDocType, SylvieSettings>,
    databaseSettings: SylvieDatabaseSettings
): Promise<RxStorageInstanceSylvie<RxDocType>> {
    const internals: SylvieStorageInternals = {};

    const broadcastChannelRefObject: DeepReadonly<any> = {};


    if (params.multiInstance) {
        const leaderElector = getSylvieLeaderElector(
    ...
    const instance = new RxStorageInstanceSylvie(
        params.databaseInstanceToken,
        storage,
        params.databaseName,
